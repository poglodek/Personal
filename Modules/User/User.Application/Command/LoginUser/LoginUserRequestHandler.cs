...
public class LoginUserRequestHandler(IUserRepository repository, ILogger<LoginUserRequestHandler> logger, 
    IPasswordHasher<Domain.Entity.User> passwordHasher,IJsonWebTokenManager jsonWebTokenManager, TimeProvider timeProvider) : IRequestHandler<LoginUserRequestCommand, JwtTokenDto>
{
    public async Task<JwtTokenDto> Handle(LoginUserRequestCommand requestCommand, CancellationToken cancellationToken)
    {
        var user = await repository.GetByEmail(requestCommand.Email, cancellationToken);
        
        await Task.Delay(Random.Shared.Next(300, 500), cancellationToken: cancellationToken);
        
        if (user is null)
        {
            logger.LogError("User with email {Email} not found", requestCommand.Email);
            throw new UserNotFoundException(requestCommand.Email);
        }
        
        var result = passwordHasher.VerifyHashedPassword(user, user.Password.Hash, requestCommand.Password + user.Password.Salt);

        if (result == PasswordVerificationResult.Failed)
        {
            logger.LogError("User with email {Email} typed bad password", requestCommand.Email);
            throw new UserNotFoundException(requestCommand.Email);
        }
        
        if (user.Activated is null)
        {
            logger.LogError("User with id {id} has no Activated account", user.Id);
            throw new UserNotActivated(user.Id);
        }
        
        if(user.Blocked is not null)
        {
            logger.LogError("User with id {id} has blocked account reason: {reason}", user.Id, user.Blocked.Reason);
